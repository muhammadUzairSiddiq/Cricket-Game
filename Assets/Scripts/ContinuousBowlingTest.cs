...
{
    /// <summary>
    /// Continuous Bowling Test System
    /// Automatically bowls ball to target, waits 3 seconds, returns to original position, and repeats
    /// </summary>
    public class ContinuousBowlingTest : MonoBehaviour
    {
        [Header("Test Setup")]
        ...
        void SetupBallPhysics()
        {
            ...
            ballRigidbody.linearDamping = 0.05f; // Reduced drag for better ball movement
            ballRigidbody.angularDamping = 0.02f; // Reduced angular drag
            ballRigidbody.useGravity = useRealisticPhysics;
            ballRigidbody.collisionDetectionMode = CollisionDetectionMode.Continuous;
            
            // 🎯 NEW: Add air resistance for more realistic cricket ball physics
            if (useRealisticPhysics)
            {
                // Cricket balls experience air resistance that affects their trajectory
        ...
        IEnumerator BowlToTarget()
        {
            ...
            // Calculate required Y velocity for arc
            float heightDifference = targetPosition.y - startPosition.y;
            float requiredYVelocity = (heightDifference + arcHeight + 0.5f * gravity * timeToReach * timeToReach) / timeToReach;
        ...
        /// <summary>
        /// Move ball using kinematic movement (when realistic physics is disabled)
        /// </summary>
