...
public class TargetDragger : MonoBehaviour
{
    ...
    void UpdateMovementLimitsFromPitchingArea()
    {
        if (pitchingAreaBounds != null)
        {
            ...
            Debug.Log($"🏏 Updated movement limits:");
            Debug.Log($"🏏 Sideways: ±{maxSideDistance}m");
            Debug.Log($"🏏 Forward/Backward: ±{maxUpDistance}m");
        }
    }
    ...
    void FindWickets()
    {
        ...
        if (wickets.Length >= 2)
        {
            ...
            Debug.Log($"🏏 Found wickets: Umpire={umpireWicket.name}, Batsmen={batsmenWicket.name}");
        }
        else
        {
            ...
            {
                Debug.Log($"🏏 Found wickets by name: Umpire={umpireWicket.name}, Batsmen={batsmenWicket.name}");
            }
            else
            {
    ...
    {
        targetPosition = originalPosition;
        Debug.Log("🏏 Target reset to original position");
    }
    ...
    {
        targetPosition = ConstrainPosition(newPosition);
        Debug.Log($"🏏 Target moved to: {targetPosition}");
    }
